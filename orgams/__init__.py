...
#Socioecon
    edad = models.IntegerField(label='Edad:', min=13, max=100)
...
class Donacion(Page):
    ...
    def vars_for_template(p: Player):
        ...
        costo_ronda = 0.75 if Ronda() < 11 else 0
...
class FinRonda(Page):
    ...
    @staticmethod
    def vars_for_template(p: Player):
        p.pago_final = p.payoff = (round((Pago_Final(p)*1)/3,1)+5)
